...
const scopeConfig = {
    DEPARTMENT: { idKey: "departmentId", resource: "department", title: "Department" },
    DIVISION: { idKey: "divisionId", resource: "division", title: "Division" },
    SECTION: { idKey: "sectionId", resource: "section", title: "Section" },
};
...
export default function ManagerDashboard() {
    ...
    useEffect(() => {
        ...
        async function loadDashboard() {
            try {
                setLoading(true);
                setError("");
                const response = await Axios.get(`/manager/${scopeId}/${scopeMeta.resource}`);
                if (!cancelled) setSnapshot(response?.data?.data || null);
            } catch (caughtError) {
                console.error(caughtError);
                if (!cancelled) setError("Unable to load your scoped analytics right now.");
            } finally {
    ...
    const summary = useMemo(() => {
        ...
        return {
            ...
            openCases: countByStatus.OPEN || 0,
            inProgressCases: (countByStatus.IN_PROGRESS || 0) + (countByStatus.ASSIGNED || 0),
            closedCases: (countByStatus.CLOSED || 0) + (countByStatus.RESOLVED || 0),
            avgRating: "—",
            totalStaff:
    ...
    const statusCases = useMemo(() => {
        const countByStatus = snapshot?.caseMetrics?.casesByStatus || {};
        const buildList = (status, count) => Array.from({ length: Number(count || 0) }, () => ({ status }));

        return [
            ...buildList("OPEN", countByStatus.OPEN || summary.openCases),
            ...buildList("ASSIGNED", countByStatus.ASSIGNED || 0),
            ...buildList("IN_PROGRESS", countByStatus.IN_PROGRESS || 0),
            ...buildList("PENDING", countByStatus.PENDING || 0),
            ...buildList("ESCALATED", countByStatus.ESCALATED || 0),
            ...buildList("RESOLVED", countByStatus.RESOLVED || 0),
            ...buildList("CUSTOMER_CONFIRMATION", countByStatus.CUSTOMER_CONFIRMATION || 0),
            ...buildList("CLOSED", countByStatus.CLOSED || summary.closedCases),
        ];
    }, [snapshot, summary.closedCases, summary.openCases]);
