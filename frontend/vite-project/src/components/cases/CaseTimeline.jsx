...
const findAssignedName = (item, caseDetails) => {
  // prefer a snapshot name from the history row if available
  ...
  if (assigned && assigned.id === newAgentId) return `${assigned.firstName || ""} ${assigned.lastName || ""}`.trim();
  return null;
};
...
export default function CaseTimeline({ history = [], caseDetails = null }) {
  const timeline = Array.isArray(history)
    ? (() => {
        ...
        // Deduplicate identical consecutive events (same from/to status, priorities, agent ids, and no note)
        const deduped = [];
  ...
  if (timeline.length === 0) {
    return <div className="text-center py-10 text-slate-500">No timeline available.</div>;
  }

  return (
    <div className="relative border-l-2 border-navy-200 ml-3">
      {timeline.map((item) => {
        const fromStatus = normalizeText(item?.fromStatus ?? item?.oldStatus ?? item?.previousStatus);
        const toStatus = normalizeText(item?.toStatus ?? item?.newStatus ?? item?.status);
        const oldPriority = normalizeText(item?.oldPriority ?? item?.previousPriority);
        ...
        return (
          <div key={item.id || `${fromStatus}-${toStatus}-${oldPriority}-${newPriority}-${newAgentId}`} className="relative pl-8 pb-10">
            <span className="absolute -left-[10px] top-1 w-4 h-4 rounded-full bg-navy-600" />
            <div className="bg-slate-50 rounded-xl p-4 border border-slate-200">
              ...
              {!hasStatusChange && !isOpenCreation && item?.status && (
                <p className="mt-3 text-slate-700 font-medium">{formatStatus(item.status)}</p>
              )}

              {isOpenCreation && (
                <div className="mt-2 text-slate-700">
                  <p>Case submitted successfully</p>
                  <p>Created by: {getDisplayName(item?.changedBy ?? item?.actor ?? caseDetails?.customer)}</p>
                  <p>Create date: {formatDateStr(getTimelineDate(item))}</p>
                  <p>Create time: {formatTimeStr(getTimelineDate(item))}</p>
                </div>
              )}
              ...
              {hasAssignmentChange && (
                <div className="mt-3 text-slate-700">
                  <p>Assigned to: {item?.newAgentName || findAssignedName(item, caseDetails) || "(staff)"}</p>
                  <p>Assigned by: {getDisplayName(item?.changedBy ?? item?.actor ?? null)}</p>
                </div>
              )}

              {item?.note && (
                <p className="mt-3 text-slate-600">{item.note}</p>
              )}
