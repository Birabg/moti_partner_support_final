...
const exportPdf = async (req, res, next) => {
    try {
        ...
        const file = actor?.partyType === "CUSTOMER"
            ? await (0, export_service_1.generatePdf)(actor.userId, "CUSTOMER")
            : await (0, export_service_1.generatePdf)();
...
const exportExcel = async (req, res, next) => {
    try {
        const actor = req.user;
        const file = actor?.partyType === "CUSTOMER"
            ? await (0, export_service_1.generateExcel)(actor.userId, "CUSTOMER")
            : await (0, export_service_1.generateExcel)();
...
const exportCsv = async (req, res, next) => {
    try {
        const actor = req.user;
        const file = actor?.partyType === "CUSTOMER"
            ? await (0, export_service_1.generateCsv)(actor.userId, "CUSTOMER")
            : await (0, export_service_1.generateCsv)();
