...
exports.createStatusHistory = createStatusHistory;
// tx is the transaction client (prisma instance or tx inside $transaction)
async function createStatusHistory(tx, payload) {
    if (!tx || !payload || !payload.caseReportId)
        throw new Error("Invalid parameters for createStatusHistory");
    ...
    if (last) {
        const fieldsMatch = (String(last.fromStatus || '') === String(payload.fromStatus || '') &&
    ...
    try {
        if ((!oldAgentName || !newAgentName) && tx && typeof tx.staff !== 'undefined') {
            if (!oldAgentName && payload.oldAgentId) {
                ...
                if (s)
                    oldAgentName = `${s.firstName || ''} ${s.lastName || ''}`.trim();
            }
            if (!newAgentName && payload.newAgentId) {
                ...
                if (s2)
                    newAgentName = `${s2.firstName || ''} ${s2.lastName || ''}`.trim();
            }
        }
    }
    catch (e) {
