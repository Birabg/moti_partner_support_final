...
exports.getSectionAnalytics = void 0;
...
const getSectionAnalytics = async (req, res, next) => {
    try {
        ...
        let targetSecId = requestedSecId || managedSecId;
        if (!isSystemAdmin) {
            if (!managedSecId) {
                throw new error_1.ForbiddenError("Access Denied: You are not assigned as a Section Manager.");
            }
            if (requestedSecId && requestedSecId !== managedSecId) {
                throw new error_1.ForbiddenError("Access Denied: You can only view analytics for your assigned section.");
            }
            targetSecId = managedSecId;
        }
        if (!targetSecId) {
            throw new error_1.NotFoundError("Section ID is required.");
        }
        const finalSecId = targetSecId;
        const sectionData = await database_1.prisma.section.findUnique({
            where: { id: finalSecId },
            include: sectionAnalyticsInclude,
        });
        if (!sectionData) {
            throw new error_1.NotFoundError("Section record not found.");
        }
        ...
        const staffMembers = (section.staff || []).map((m) => ({
            id: m.id,
